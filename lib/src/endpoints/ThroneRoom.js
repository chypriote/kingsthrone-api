...
class ThroneRoom extends GoatResource_1.GoatResource {
    //Levies
    async getCoins() {
        await this.request({ 'user': { 'jingYing': { 'jyid': ThroneRoom_1.LEVEE_TYPE.COIN } }, 'rsn': '7cogcyxgoy' });
    }
    async getArmy() {
        await this.request({ 'user': { 'jingYing': { 'jyid': ThroneRoom_1.LEVEE_TYPE.ARMY } }, 'rsn': '6sxwyglskxg' });
    }
    async getGrain() {
        await this.request({ 'user': { 'jingYing': { 'jyid': ThroneRoom_1.LEVEE_TYPE.GRAIN } }, 'rsn': '3hkfhezfke' });
    }
    async loopAllLevies() {
