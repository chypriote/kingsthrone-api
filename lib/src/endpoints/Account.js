...
class Account extends GoatResource_1.GoatResource {
    ...
    async doMainQuestTask(id) {
        await this.request({ 'task': { 'taskdo': { 'id': id } }, 'rsn': lodash_1.sample([
    ...
    async doCampaignGuide(smap, bmap, mmap) {
        await this.request({ 'guide': { 'guide': { smap, bmap, mmap } }, 'rsn': lodash_1.sample([
                '6wugwxgwly', '6wugwxgwly', '8maoemkekio', '8jmaikxvakk', '2anxayxmmx', '6sxwyglppbg', '1ktuikaiwek',
            ]) });
    }
    async doGuide(id) {
        await this.request({ 'guide': { 'guide': { 'gnew': id } }, 'rsn': lodash_1.sample([
